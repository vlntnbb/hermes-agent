...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let hermesPath = "/Users/val.vasilevsky/.local/bin/hermes"
    private let projectPath = "/Users/val.vasilevsky/Git/hermes/hermes-agent"
    private let hermesHome = "/Users/val.vasilevsky/.hermes"
    ...
    @objc private func openDashboard() {
        DispatchQueue.global(qos: .userInitiated).async {
            if !self.isDashboardRunning() {
                _ = self.runShell("cd \(self.quoted(self.projectPath)) && nohup \(self.quoted(self.hermesPath)) dashboard --no-open > \(self.quoted(self.hermesHome + "/logs/dashboard-menubar.log")) 2>&1 &", wait: true)
    ...
    @objc private func startDashboard() {
        runCommandAsync("Start Dashboard") {
            ...
            return self.runShellResult("cd \(self.quoted(self.projectPath)) && nohup \(self.quoted(self.hermesPath)) dashboard --no-open > \(self.quoted(self.hermesHome + "/logs/dashboard-menubar.log")) 2>&1 &", wait: true)
        }
    }
    ...
    private func runShellResult(_ command: String, wait: Bool) -> CommandResult {
        ...
        process.environment = [
            ...
            "PATH": "/Users/val.vasilevsky/Git/hermes/hermes-agent/venv/bin:/Users/val.vasilevsky/.local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
        ]
